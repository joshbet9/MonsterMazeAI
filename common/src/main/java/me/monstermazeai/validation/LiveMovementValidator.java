 ...
 *
 * It compares consecutive observations against the action issued between them.
 * This is deliberately observational: Minecraft remains the authority for
 * actual physics. A sample is valid when positions/velocities are finite,
 * displacement is bounded, and commanded movement produces measurable motion.
 */
public final class LiveMovementValidator {
    ...
    private long jumpSamples;
    private long abilitySamples;
    ...
    public void observe(LegacyWorldObservation observation, LegacyAction action) {
        ...
        if (previous != null && previousAction != null && observation.inMonsterMaze
                && previous.inMonsterMaze) {
            samples++;
            ...
            if (!finite(dx) || !finite(dz) || !finite(dy)
                ...
                if (horizontal > 1.0e-4
                        ...
                        || Math.abs(previousAction.strafe) > 1.0e-6)) {
                    movingSamples++;
                }
