...
class PlannerValidationTest {
    ...
    private static GameState state(double x,double z,int timerTicks) {
        ...
        s.player.x=x; s.player.z=z; s.player.y=0; s.player.grounded=true;
    ...
    private static BeamSearchPlanner planner(GameState s,int horizon,int width) {
        return new BeamSearchPlanner(new Simulator(new LegacyMazePhysics(),new MonsterSimulator(s.maze,new Random(1234),0.07),new CollisionModel()),new Heuristic(),horizon,width);
    }
    @Test void reachesPadInOpenSpace() {
        ...
        assertTrue(plan.resultingState().alive); assertTrue(plan.sequence().length()<=60);
    }
    ...
    @Test void collisionDamageIsVisibleToPlanner() {
        ...
        Simulator simulator=new Simulator(new LegacyMazePhysics(),new MonsterSimulator(s.maze,new Random(1),0.07),new CollisionModel());
